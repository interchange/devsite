...
var Presentazion = (function() {"use strict"; return {
    init : function() {
        ...
        this.current_slide = this.slide_from_url();
        ...
        this.bind_hashchange_events();
        $(window).resize($.proxy(function() {
            this.set_text_size(this.current_slide);
        }), this);
        this.change_slide(this.current_slide, true);
    },

    slide_from_url : function() {
        var this_hash = window.location.hash;
        var result = this_hash.match(/^#slide(\d+)$/);
    ...
    change_slide : function(num, force) {
        if (num === null || num < 0 || num > this.max_slide) return;
        if (num === this.current_slide && !force) return;
        $(".slide:eq(" + this.current_slide + ")").hide();
        this.current_slide = num;
        $(".slide:eq(" + this.current_slide + ")").show();
        this.set_text_size(this.current_slide);
        window.location.hash = (this.current_slide === 0 ? '' : 'slide' + this.current_slide);
    },
    ...
    // has_value, is_parent_or_self, and bind_click_events
    // adapted from S5 v1.1 slides.js -- released into the Public Domain
    // http://www.meyerweb.com/eric/tools/s5/credits.html

    has_value : function(object, value) {
        if (!object) return false;
        return (object.search('(^|\\s)' + value + '(\\s|$)') != -1);
    },

    is_parent_or_self : function(element, id) {
        if (element == null || element.nodeName == 'BODY') return false;
        else if (element.id == id) return true;
        else return this.is_parent_or_self(element.parentNode, id);
    },
    ...
    bind_click_events : function() {
        $(document).click($.proxy(function(e) {
            ...
            if (target.getAttribute('href') != null
                || this.has_value(target.rel, 'external')
                || this.is_parent_or_self(target, 'controls')
                || this.is_parent_or_self(target, 'embed')
                || this.is_parent_or_self(target, 'object')
            ) return true;
    ...
    bind_hashchange_events : function() {
        $(window).bind('hashchange', $.proxy(function(e) {
            this.change_slide(this.slide_from_url());
            return true;
        }, this));
    },
